...
def simulate_lc712_from_powerspectrum(mean=1551,rms=np.sqrt(0.6657E+05)/1551,dt=2,
                                   plot_results=0):
...
#model  lorentz + lorentz +  lorentz  +   lorentz   +   lorentz   +   lorentz   +   lorentz   +   powerlaw   +   powerlaw
    ...
    spectrum = lorentzian(w, *xspec_pars[0:3])+lorentzian(w, *xspec_pars[3:6])+ lorentzian(w, *xspec_pars[6:9])+lorentzian(w,*xspec_pars[9:12])+lorentzian(w,*xspec_pars[12:15])+lorentzian(w,*xspec_pars[15:18])+lorentzian(w,*xspec_pars[18:21])+po(w,*xspec_pars[21:23])+po(w,*xspec_pars[23:26])


    lc = sim.simulate(spectrum)
    tmp=np.random.normal(lc.counts-lc.counts,130)
...
for i in range(10):
    ...
    lc67.counts=gaussian_filter1d(lc67.counts,sigma=2)
...
plot_ccf('/Users/s.bykov/work/xray_pulsars/rxte/results/out90089-11-03-01G/products/sa_data_lc_0.1sec/ccf_0.9.qdp',ax_ccf)
